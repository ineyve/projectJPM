                        ...
                        @foreach ($requests as $request)
                            <tr>
                                ...
                                @if($request->status == 0)
                                    ...
                                    <td><a class="btn btn-xs btn-info" href="{{route('requests.status', ['request' => $request, 'status' => 1, 'from' => 0])}}">Progress</a>
                                @endif
                                @if($request->status == 1)
                                        <a href="{{route('requests.details', $request)}}">In progress</a></td>
                                    <td><a class="btn btn-xs btn-primary" href="{{route('requests.status', ['request' => $request, 'status' => 2, 'from' => 0])}}">Ready</a>
                                @endif
                                @if($request->status == 2)
                                        <a href="{{route('requests.details', $request)}}">Ready</a></td>
                                    <td><a class="btn btn-xs btn-success" href="{{route('requests.status', ['request' => $request, 'status' => 3, 'from' => 0])}}">Complete</a>
                                @endif
                                @if($request->status == 3)
